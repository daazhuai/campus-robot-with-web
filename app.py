...
def register():
    ...
    if request.method == 'GET':
        ...
        # 拼凑 sql 语句
        sql = f'insert into message (username,password,college,idnumber) values ("{username}","{password}","{college}","{id_number}")'
        # 执行 sql 语句
...
def find_back():
    if request.method == 'GET':
        ...
        sql = f'select * from message where username = "{username}" and idnumber = "{id_number}" and college = "{college}"'
        db = Mysql()
        db.cursor.execute(sql)
        user = db.cursor.fetchone()
        print(user)
...
@app.route('/user/talk/answer', methods=['GET', 'POST'])
def answer_serveices():
    text = request.json.get("contest")
    print(text)
    reply = answer.unit_chat(text)
    print(reply)
    return reply
